...
@RoutePage()
class SocietyBasicInfoScreen extends StatefulWidget {
  const SocietyBasicInfoScreen({super.key});

  @override
  State<SocietyBasicInfoScreen> createState() => _SocietyBasicInfoScreenState();
}

class _SocietyBasicInfoScreenState extends State<SocietyBasicInfoScreen> {
  ...
  Future<void> _validateAndSendOTP() async {
    ...
    // Store email for the next screen
    _email = email;
    ...
    try {
      ...
      if (isValid) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Invalid email domain for ${_selectedCollege!.name}'),
          ),
        );
      }
    } catch (e) {
  ...
  void _showUniversitySelectionDialog() async {
    ...
    try {
      ...
      if (universities.isEmpty) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('No universities found')));
        return;
      }
  ...
  void _showCollegeSelectionDialog() async {
    ...
    try {
      ...
      if (colleges.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('No colleges found for this university'),
          ),
        );
        return;
      }
